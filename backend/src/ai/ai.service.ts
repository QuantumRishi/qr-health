...
export type AIProvider = 'gemini' | 'groq' | 'sarvam' | 'openai' | 'anthropic' | 'local';
...
export class AiService {
  ...
  async chat(userId: string, message: string): Promise<AIResponse> {
    ...
    try {
      ...
      this.logger.warn(`AI provider ${this.currentProvider} failed, using fallback`);
...
Always end responses with encouragement and remind users you're here to support their recovery journey.`;
  ...
  private async callGemini(systemPrompt: string, message: string): Promise<string> {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text || this.generateLocalResponse(message);
  }

  private async callGroq(systemPrompt: string, message: string): Promise<string> {
    ...
    const data = await response.json();
    return data.choices?.[0]?.message?.content || this.generateLocalResponse(message);
  }

  private async callSarvam(systemPrompt: string, message: string): Promise<string> {
  ...
  private async callOpenAI(systemPrompt: string, message: string): Promise<string> {
  ...
  private async callAnthropic(systemPrompt: string, message: string): Promise<string> {
